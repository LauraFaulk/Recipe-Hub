...
async function loadDeps(t: import('node:test').TestContext) {
  try {
    ...
    t.skip('Skipping ingest telemetry failure test because route dependencies are unavailable in this environment.');
    return null;
  }
}

test('ingest 400 request records failed telemetry event', async (t) => {
